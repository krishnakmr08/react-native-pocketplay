...
import { Reaction } from "@/types/play";
...
const LiveReactions: FC<Props> = ({ initialValue }) => {
  ...
  const addEmoji = useCallback((emoji: string) => {
    const newReaction: AnimatedHeart = {
    ...
    Animated.parallel([
      ...
      setHearts((prev) => prev.filter((heart) => heart.id !== newReaction.id));
    });
  }, []);

  useEffect(() => {
    const handleReaction = (reactionData: Reaction) => {
